...
public class ShowUserRents implements Operation {
    ...
    public void operation(Database database, JFrame f, User user) {
        ...
        frame.getContentPane().setBackground(new Color(101, 11, 145));
        frame.setLayout(new BorderLayout());
        JLabel title = new JLabel("Rents", 35);
        title.setBorder(BorderFactory.createEmptyBorder(20, 0, 0, 0));
        frame.add(title, "North");
        String[] header = new String[]{"ID", "Name", "Email", "Tel", "Car ID", "Car", "Date Time", "Hours", "Total", "Status"};
        ArrayList<Rent> rents = new ArrayList();
        ArrayList<Integer> carIDs = new ArrayList();

        try {
            String select = "SELECT * FROM `rents` WHERE `User` = '" + this.userID + "';";
            ...
            String selectUser = "SELECT * FROM `users` WHERE `ID` = '" + this.userID + "';";
        ...
        panel.getViewport().setBackground((Color)null);
