...
public class AddNewAccount implements Operation {
    ...
    public void operation(final Database database, JFrame f, User u) {
        final JFrame frame = new JFrame("Create New Account");
        frame.setSize(600, 600);
        frame.setLocationRelativeTo(f);
        frame.getContentPane().setBackground(new Color(186, 179, 135));
            ...
            public void actionPerformed(ActionEvent e) {
                if (firstName.getText().equals("")) {
                    ...
                    try {
                        ...
                        String insert = "INSERT INTO `users`(`ID`, `FirstName`, `LastName`, `Email`, `PhoneNumber`, `Password`, `Type`) VALUES ('" + ID + "','" + firstName.getText() + "','" + lastName.getText() + "','" + email.getText() + "'," + "'" + phoneNumber.getText() + "','" + password.getText() + "','" + AddNewAccount.this.accType + "');";
                        database.getStatement().execute(insert);
                        JOptionPane.showMessageDialog(frame, "Account created successfully");
                        if (AddNewAccount.this.accType == 0) {
